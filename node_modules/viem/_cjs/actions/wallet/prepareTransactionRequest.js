...
const supportsFillTransaction = new lru_js_1.LruMap(128);
async function prepareTransactionRequest(client, args_) {
    const attemptFill = supportsFillTransaction.get(client.uid) !== false &&
        ['fees', 'gas'].some((parameter) => args_.parameters?.includes(parameter));
    const fillResult = attemptFill
        ? await (0, getAction_js_1.getAction)(client, fillTransaction_js_1.fillTransaction, 'fillTransaction')(args_)
            .then((result) => {
            ...
            .catch((e) => {
            const error = e;
            if (error.cause.name === 'MethodNotFoundRpcError' ||
                error.cause.name === 'MethodNotSupportedRpcError')
                supportsFillTransaction.set(client.uid, false);
            return args_;
        })
        : args_;
    const { account: account_ = client.account, blobs, chain, gas, kzg, nonce, nonceManager, parameters = exports.defaultParameters, type, } = fillResult;
